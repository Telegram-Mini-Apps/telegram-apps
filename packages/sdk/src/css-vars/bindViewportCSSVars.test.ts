...
it(
  'should set --tg-viewport-height = viewport.height, --tg-viewport-width = viewport.width, --tg-viewport-stable-height = viewport.stableHeight',
  () => {
...
it(
  'should update --tg-viewport-height, --tg-viewport-width, --tg-viewport-stable-height according to the values, received in the Viewport change events',
  () => {
...
it('should stop updating variables, if returned function was called', () => {
  ...
  const cleanup = bindViewportCSSVars(viewport);
  ...
  expect(setCSSPropertySpy).toHaveBeenCalledTimes(6);
  cleanup();
  dispatchWindowMessageEvent('viewport_changed', {
    height: 1000,
    is_state_stable: true,
