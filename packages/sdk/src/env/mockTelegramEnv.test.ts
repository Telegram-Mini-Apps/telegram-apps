...
describe('iframe', () => {
  it('should wire window.parent.postMessage', () => {
    ...
    expect(postMessage).toHaveBeenCalledWith('{"eventType":"web_app_expand"}');
  });

  it('should emit "theme_changed" event for "web_app_request_theme" method', () => {
    const postMessage = vi.fn();
    createWindow({
      parent: { postMessage } as any,
      env: 'iframe',
  ...
  it('should emit "viewport_changed" event for "web_app_request_viewport" method', () => {
    const postMessage = vi.fn();
    createWindow({
      parent: { postMessage } as any,
      innerWidth: 1000,
...
describe('external notify', () => {
  it('should wire window.external.notify', () => {
    const notify = vi.fn();
    createWindow({ external: { notify } as any });
    mockTelegramEnv(lp);

    postEvent('web_app_expand');
    expect(notify).toHaveBeenCalledOnce();
    expect(notify).toHaveBeenCalledWith('{"eventType":"web_app_expand"}');
  });
...
describe('default', () => {
  ...
  it('should emit "theme_changed" event for "web_app_request_theme" method', () => {
    createWindow({ TelegramWebviewProxy: { postEvent: vi.fn() } } as any);
    mockTelegramEnv(lp);
  ...
  it('should emit "viewport_changed" event for "web_app_request_viewport" method', () => {
    createWindow({ innerWidth: 1000, innerHeight: 2000 });
    mockTelegramEnv(lp);
