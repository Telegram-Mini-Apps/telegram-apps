...
import type { RetrieveInitDataFn, RetrieveRawInitDataFn } from '@/features/types.js';

function instantiate({
  retrieveInitData = () => E.left(new Error('Init data missing')),
  retrieveRawInitData = () => E.right(O.none),
}: {
  retrieveInitData?: RetrieveInitDataFn<unknown>;
  retrieveRawInitData?: RetrieveRawInitDataFn<unknown>;
} = {}) {
...
describe('restore', () => {
  it('should set state based on init data from launch params', () => {
    ...
    const component = instantiate({
      retrieveInitData: () => E.right(initData),
