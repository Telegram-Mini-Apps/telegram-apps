...
// 导航器将添加一个新条目，当前条目变为 /home/license
```
...
// 导航器将添加一个新条目，当前条目变为 /home/blog?id=1
```
...
// { pathname: '/b', hash：''，搜索：'?b-query=1', id: 'dd82' },
// ].
```
...
事件只有`change`。 变化 "事件的有效载荷是一个对象，包含
以下属性：

- `navigator：BrowserNavigator`：相关导航器实例。
- delta：数字：当前活动项目光标的 delta。
