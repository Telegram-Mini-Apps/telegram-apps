...
Привязки React для [клиентского SDK](tma-js-sdk/about). Включают в себя хуки, компоненты и другие полезные инструменты, которые позволяют использовать React вместе с клиентским SDK для веб-приложений. Автоматически отслеживает изменения в компонентах SDK.

<LibraryBadges pkg={'@tma.js/sdk-react'}/>

## Установка

Прежде всего, предполагается, что вы уже установили пакет `react`, поскольку он является обязательной зависимостью этого пакета. Установка самого SDK не требуется, так как он уже включен в `@tma.js/sdk-react`.

<NpmInstall pkg={'@tma.js/sdk-react'}/>
...
Этот хук вернет результат выполнения функции [`retrieveLaunchParams`](tma-js-sdk/about#launch-parameters)
